...
var dTP=null;
function cA(){
for(var i=0;i<TR.length;i++)Timer.clear(TR[i]);
TR=[];
}
...
function ui(bri,rgb,trans){light(CFG.ui,bri>0,bri,rgb,trans);}
function amb(bri,rgb,trans){light(CFG.ambient,bri>0,bri,rgb,trans);}
function boot(){
...
if(idx===2)amb(30,CL.warm,step*4);
idx++;
lt(step,nS);
}else{
lt(500,function(){sS("ready");});
}
}
nS();
}
...
lt(TM.breatheIn,function(){ui(0,CL.white,TM.breatheOut);});
}
br();
...
var dur=seq[i];var isOn=on;
ui(isOn?15:0,CL.white,0);
if(isOn)amb(25,CL.blue,50);else amb(18,CL.cool,100);
i++;on=!on;
lt(dur,next);
}
next();
...
lt(TM.dr,function(){amb(28,CL.neutral,TM.dr);});
}
dr();
lt(TM.dr*2,dr,true);
}
...
amb(12,CL.warm,100);
function blink(i){
if(i>=3){lt(150,function(){amb(20,CL.warm,300);});return;}
ui(25,CL.amber,0);
lt(150,function(){ui(0,CL.amber,0);lt(150,function(){blink(i+1);});});
}
blink(0);
}
wn();
...
if(i>=3)return;
ui(30,CL.red,0);
lt(120,function(){ui(0,CL.red,0);lt(120,function(){blink(i+1);});});
}
blink(0);
}
al();
...
function br(){
ui(3,CL.white,half);
lt(half,function(){ui(0,CL.white,half);});
}
br();
...
var PT={
boot:boot,ready:ready,touch:touch,pairing:pairing,
active:active,low_liquid:lowLiquid,error:error,
night:night,goodbye:goodbye
};
function sS(newState){
if(!newState||newState===state)return;
if(!PT[newState]){print("[Cury]Unknown state:"+newState);return;}
cA();
state=newState;
...
if(slotData.vial&&slotData.vial.level>=0&&slotData.vial.level<=CFG.lowLevelThreshold){
...
if(r.errors[i]!=="acc_bus")hErr.push(r.errors[i]);
}
}
if(hErr.length>0){
if(!hE){
...
if(!hE){hE=true;hL=false;sS("error");}
return;
}
if(lR==="low"||rR==="low"){
if(!hL&&!hE){hL=true;sS("low_liquid");}
return;
}
if(hE||hL){
...
if(!e&&r&&r.value&&r.value!==state)sS(r.value);
});
}
Shelly.call("KVS.Get",{key:CFG.kvsKey},function(r,e){
